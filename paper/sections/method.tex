...
\begin{tikzpicture}[
    ...
    scorebox/.style={draw=orange!80!black, fill=yellow!15, rounded corners=3pt,
        minimum height=1.2cm, font=\small, align=center, line width=0.8pt},
...
\node[font=\footnotesize\bfseries, text=blue!70!black, anchor=west]
    at (-1.3, 4.2) {\textsc{Seismic Wave Detection Model}};
\node[font=\footnotesize\bfseries, text=red!70!black, anchor=west]
    at (-1.3, 0.2) {\textsc{Seismic Environment}};
% ===================== TOP STREAM (blue) =====================
\node[blueinput, minimum width=2.2cm] (seis) at (0, 3) {Seismogram};
\node[bluemodel, minimum width=3cm] (pn) at (3.8, 3)
    {\textbf{PhaseNet}\\[-2pt]
     {\scriptsize\textit{original weights (Northern California)}}};
\node[blueinput, minimum width=2.8cm] (pick) at (7.8, 3)
    {Wave arrival pick\\[-2pt] with confidence $c$};
% Top stream arrows
\draw[bluearrow] (seis) -- (pn);
\draw[bluearrow] (pn) -- (pick);
% ===================== BOTTOM STREAM (red) =====================
\node[redinput, minimum width=2.2cm] (cat) at (0, -1)
    {Earthquake\\[-2pt] Catalogue};
\node[redmodel, minimum width=2.6cm] (hmm) at (3.3, -1)
    {\textbf{HMM}\\[-2pt]
     {\scriptsize Quiet / Active / Decaying regimes}};
\node[redmodel, minimum width=2.6cm] (bb) at (6.6, -1)
    {\textbf{Beta-Bernoulli}\\[-2pt]
     {\scriptsize PhaseNet reliability per regime}};
\node[redinput, minimum width=2.8cm] (sigma) at (10.2, -1)
    {Uncertainty in\\[-2pt] PhaseNet estimate, $\sigma_t$};
% Bottom stream arrows
\draw[redarrow] (cat) -- (hmm);
\draw[redarrow] (hmm) -- node[above, font=\scriptsize, text=red!70!black]
    {regime posterior, $\pi_t(r)$} (bb);
\draw[redarrow] (bb) -- (sigma);
% ===================== CONVERGENCE =====================
% Combined score
\node[scorebox, minimum width=2.6cm] (cs) at (12.5, 1)
    {\textbf{Combined Score}\\[-2pt]
     {\scriptsize $s = c \cdot \max\!\big(0,\; 1 - \gamma \cdot \sigma_t\big)$}};
% Gamma parameter
\node[parambox, minimum width=2cm] (gamma) at (12.5, 3)
    {Scaling factor, $\gamma$};
\draw[grayarrow] (gamma) -- (cs);
% Blue: pick -> down and right -> into combined score top-left
\draw[bluearrow]
    (pick.east) -- ++(0.6, 0) coordinate (bturn)
    -- (bturn |- cs.west) -- ([yshift=3pt]cs.west)
    node[pos=0.35, right, font=\small\itshape, text=blue!70!black] {$c$};
% Red: sigma -> up and right -> into combined score bottom-left
\draw[redarrow]
    (sigma.east) -- ++(0.3, 0) coordinate (rturn)
    -- (rturn |- cs.west) -- ([yshift=-3pt]cs.west)
    node[pos=0.35, right, font=\scriptsize\itshape, text=red!70!black]
    {$1 - \gamma \cdot \sigma_t$};
% ===================== DECISION =====================
\node[decision] (dec) at (15.5, 1) {$s <$ deferral\\threshold?};
\draw[blackarrow] (cs) -- (dec);
% Defer (up)
\node[outcome, draw=red!70!black, fill=red!10, text=red!70!black]
    (defer) at (15.5, 3.2) {Defer};
\draw[redarrow] (dec) -- (defer);
% Accept (down)
\node[outcome, draw=green!50!black, fill=green!8, text=green!50!black]
    (accept) at (15.5, -1) {Accept};
