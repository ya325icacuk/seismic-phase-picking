...
\caption{System pipeline. A seismic waveform is processed by PhaseNet to produce a wave arrival pick with confidence score~$c$. Independently, an HMM characterises the seismic environment into regimes using the earthquake catalogue, and a Beta-Bernoulli model estimates PhaseNet's reliability in each regime. The regime-weighted posterior uncertainty~$\sigma_t$ is combined with~$c$ into a deferral score~$s$, where $\gamma$ controls the sensitivity of the regime penalty. Picks scoring below the threshold are deferred to a human analyst.}
\label{fig:pipeline}
\end{figure*}
